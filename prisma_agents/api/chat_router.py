...
@router.post(
    "/start",
    tags=["Chat"],
    summary="Iniciar sesión de generación de rúbrica",
    description=(
        "Sube el PACI y el material base del estudiante, crea una sesión y dispara el workflow "
        "multi-agente en background.\n\n"
        "**Modo local:** los archivos se guardan en disco y el workflow arranca como BackgroundTask.\n\n"
        "**Modo AWS:** los archivos se suben a S3 y la Lambda `prisma-trigger` arranca el workflow "
        "llamando a `/chat/internal/run/{session_id}`. El cliente debe conectarse a "
        "`/chat/{session_id}/stream` para seguir el progreso via SSE."
    ),
